    ...
    , m_walkbutton(GPIO_BUTTON)
{
    ...
    //   poll every 10 ms
    _button.setInterval(10);
    _button.start();

    connect(&_button, SIGNAL(timeout()), this, SLOT(handleWalkPressed()));
}

void TrafficLightDevice::handleWalkPressed()
{
    m_walkbutton.updateGpio();

    if (m_walkbutton.lineState() == GpioIn::High) {
        m_trafficLight.walk();
    }
}
